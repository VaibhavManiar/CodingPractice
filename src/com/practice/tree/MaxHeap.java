...
public class MaxHeap {
    ...
    private void heapifyBottomUp(int index) {
        if((index >= 0 && parentIndex(index) >=0) &&
                (index < heap.size() && parentIndex(index) < heap.size()) &&
                heap.get(parentIndex(index)) < heap.get(index)) {
    ...
    public static void main(String[] args) {
        ...
        IntStream.range(1, 10).forEach(heap::add);
        heap.heap.forEach(System.out::println);
        System.out.println(heap.removeAndGet());
    }
}